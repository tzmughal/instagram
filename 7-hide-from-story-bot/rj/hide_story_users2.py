...
def click_checkbox(driver, wait, username, row, toggle):
    ...
    pointer-events: none -- and confirm the checked state actually changed
    ...
    clicking a stale element or, worse, double-toggling.
    """
    was_checked = is_toggle_checked(toggle)
    ...
    def state_changed(_):
        ...
        return is_toggle_checked(fresh_toggle) != was_checked
    ...
    for method in methods:
        ...
        try:
            wait.until(state_changed)
            logging.info("%s toggled the checkbox for %s.", method, username)
            return True
        except TimeoutException:
            logging.info(
...
def hide_user(driver, wait, username):
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if click_checkbox(driver, wait, username, row, toggle):
            ...
            if attempt < MAX_RETRIES:
                logging.warning(
                    "Checkbox did not toggle for %s. Retrying... (%s/%s)",
