...
def prompt_mode():
    ...
    Returns "1", "2", "3", "4", or "5":
      ...
      "3" -- custom: check everyone in check.txt, uncheck everyone in
             uncheck.txt
      "4" -- check every username previously recorded as "not found"
      "5" -- uncheck every username previously recorded as "not found"
    ...
    print(f"  4) Check not found     -- retry-check every username recorded as 'not found' in {RECORD_FILENAME}")
    print(f"  5) Uncheck not found   -- retry-uncheck every username recorded as 'not found' in {RECORD_FILENAME}")

    labels = {
        "1": "Check all",
        "2": "Uncheck all",
        "3": f"Custom ({CHECK_FILENAME} + {UNCHECK_FILENAME})",
        "4": "Check usernames not found",
        "5": "Uncheck usernames not found",
    }
...
def prompt_start_over():
    """Ask whether to resume using the existing record CSV or wipe it and
    start fresh. Returns "csv" or "scratch".
    """
    print("How should progress be handled?")
    print(f"  1) Use existing record -- skip usernames already recorded in {RECORD_FILENAME}")
    print(f"  2) Start from scratch  -- delete {RECORD_FILENAME} and reprocess everything")

    labels = {"1": "csv", "2": "scratch"}
    label_text = {"csv": "Use existing record", "scratch": "Start from scratch"}

    while True:
        choice = input("Enter 1 or 2: ").strip()
        if choice in labels:
            result = labels[choice]
            print(f"Selected: {label_text[result]}.\n")
            return result
...
def usernames_marked_not_found(record):
    """Return every username that has a "not_found" outcome in ``record``
    (as produced by ``load_record``), de-duplicated and in first-seen order.
    A username can appear in the record under more than one target state;
    it only needs to show up once here.
    """
    seen = set()
    usernames = []
    for (username, _target_checked), outcome in record.items():
        if outcome == "not_found" and username not in seen:
            seen.add(username)
            usernames.append(username)
    return usernames


def build_task_list(mode, record=None):
    """Turn the chosen mode into an ordered, de-duplicated list of
    (username, target_checked) pairs to work through.

    ``record`` (as produced by ``load_record``) is only needed for modes
    "4" and "5", which pull their usernames from the "not found" entries
    of a previous run instead of a text file.
    """
    if mode == "1":
    ...
    else:  # mode in ("4", "5") -- retry usernames previously "not found"
        not_found_usernames = usernames_marked_not_found(record or {})
        target_checked = mode == "4"
        tasks = [(username, target_checked) for username in not_found_usernames]
...
def main():
    mode = prompt_mode()
    start_choice = prompt_start_over()

    # Load the record *before* any possible deletion below -- modes "4"
    # and "5" need it to know which usernames were "not found", and mode
    # "scratch" should still wipe it clean afterwards.
    record = load_record()

    try:
        tasks = build_task_list(mode, record)
    except FileNotFoundError as error:
        print(f"Error: {error}")
        return

    if mode in ("4", "5") and not tasks:
        print(f"No usernames are recorded as 'not found' in {RECORD_FILENAME} yet. Nothing to do.")
        return
    ...
    if start_choice == "scratch":
        ...
        if os.path.exists(path):
            os.remove(path)
            print(f"Deleted {RECORD_FILENAME} -- starting fresh.\n")
        record = {}
    ...
    try:
        ...
        for username, target_checked in remaining_tasks:
            ...
            if outcome in ("checked", "unchecked", "skipped", "not_found"):
                append_record(username, target_checked, outcome)
                if outcome == "not_found":
                    print(
                        f"  [!] {username}: not found -- recorded in {RECORD_FILENAME}; "
                        f"use option 4/5 to retry it later."
                    )
            else:
                # "failed" is deliberately NOT written to the record file,
                # so it gets tried again next run instead of being skipped
                # forever.
